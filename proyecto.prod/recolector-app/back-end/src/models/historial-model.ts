...
export const obtenerHistorialDB = async (idRecolector: number) => {
  const [rows] = await pool.query<RowDataPacket[]>(
    ...
    INNER JOIN pedidos p ON dp.pedidos_idpedidos = p.idpedidos
    INNER JOIN direcciones d ON p.id_direccion = d.iddirecciones
    INNER JOIN tipo_reciclable tr ON p.tipo_reciclable_idtipo_reciclable = tr.idtipo_reciclable
    WHERE p.id_recolector = ?
